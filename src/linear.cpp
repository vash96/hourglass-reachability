...
vector<int> linear_A(const DAG & dag)
{
    int n = dag.n_nodes();
    vector<int> A(n, 1);
    ...
    // Cumulative update
    int x=-1;
    for(const auto & [i, j] : views::reverse(updates)) {
        x = max(x, i+1);
        while(x < j) {
            A[x++] = 0;
        }
...
vector<int> linear_B(const DAG & dag)
{
    int n = dag.n_nodes();
    vector<int> B(n, 1);
    vector<pair<int, int>> updates;
...
{
    const auto A = linear_A(dag);
    const auto B = linear_B(dag);
